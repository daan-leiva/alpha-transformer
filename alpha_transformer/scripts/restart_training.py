...
def main():
    args = parse_args()
    # Always get the root (repo) directory no matter where you run from
    ...
    # load tokenizer using saved model path
    ...
    # build trainer and load state
    trainer = Trainer(model=model, tokenizer=tokenizer, args=saved_args,
                      special_tokens=data.special_tokens, optimizer=optimizer,
                      scheduler=scheduler, loss_fn=loss_fn, train_loader=train_loader,
                      val_loader=val_loader, log_file=log_file)
